//Лабораторная работа 3. RC5+Вероятностный шифр. Сабитова Кристина, ИТ-41.
#include <stdio.h>
#include <iostream>
#include <conio.h>
#include <iomanip>
...
void main()
{
	...
	WORD encr1[2] = { 0xEEDB,0x6D8F};
		for (j = 0; j < 2; j++)
		{
			encr1[j] = (encr1[j] << 16) + (rand()) % (1 << 16);  //Добавляем рандомную часть к сообщению
		}
	...
	decryption(decr, encr2);
		...
		for (j = 0; j < 2; j++)
		{
			encr2[j] = (encr2[j] >> 16);  //Убираем рандомную часть
		}
	cout << "key = ";
	for (j = 0; j < b; j++)
	{
	...
	cout << "Расшифрованное сообщение:  " << hex << encr2[0] << " " << encr2[1] << endl;
